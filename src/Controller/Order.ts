...
const order = expressAsyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      SuccessHandler.sendSuccessResponse(
        res,
        "Orders placed but pending for approval",
        { orders }
      );
    } catch (error) {
      next(error);
...
const GetAllOrders = expressAsyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      SuccessHandler.sendSuccessResponse(res, "List of Orders are :", {
        getAll,
        page: page,
...
const CancelOrder = expressAsyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const userId = req.user.user.id;
      ...
      SuccessHandler.sendSuccessResponse(
        res,
        "List of Orders are after cacelling order:",
        { getAll }
      );
    } catch (error: any) {
      res
