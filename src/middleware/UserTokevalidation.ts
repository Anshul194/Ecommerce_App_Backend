...
const validateUser = expressAsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    jwt.verify(token, process.env.Access_Token_Secret as string, async (err: jwt.VerifyErrors | null, decoded: any) => {
        if (err) {
